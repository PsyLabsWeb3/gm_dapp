...
/**
 * @dev Standard ERC165 implementation used in contracts that need to support
 * interaction with the Registry (IERC165) and other contract interfaces.
 */
    ...
    function checkOnERC1155Received(
        ...
        if (to.code.length > 0) {
            try IERC1155Receiver(to).onERC1155Received(operator, from, id, value, data) returns (bytes4 response) {
                ...
                if (reason.length == 0) {
                    // Non-ERC1155 compliant receiver
                    revert IERC1155Errors.ERC1155InvalidReceiver(to);
                } else {
                    // Reverted with error message
...
abstract contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI, IERC1155Errors {
    ...
    function safeTransferFrom(address from, address to, uint256 id, uint256 value, bytes memory data) public virtual {
        ...
        if (from != sender && !isApprovedForAll(from, sender)) {
            revert IERC1155Errors.ERC1155MissingApprovalForAll(sender, from);
        }
        _safeTransferFrom(from, to, id, value, data);
    }

    /**
     * @dev See {IER1155-safeBatchTransferFrom}.
     */
    function safeBatchTransferFrom(
        address from,
        address to,
    ...
    function _update(address from, address to, uint256[] memory ids, uint256[] memory values) internal virtual {
        if (ids.length != values.length) {
            revert IERC1155Errors.ERC1155InvalidArrayLength(ids.length, values.length);
        }
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            if (from != address(0)) {
                ...
                if (fromBalance < value) {
                    revert IERC1155Errors.ERC1155InsufficientBalance(from, fromBalance, value, id);
                }
                unchecked {
    ...
    function _setApprovalForAll(address owner, address operator, bool approved) internal virtual {
        if (operator == address(0)) {
            revert IERC1155Errors.ERC1155InvalidOperator(address(0));
        }
